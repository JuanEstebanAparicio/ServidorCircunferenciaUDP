...
import juanesteban.circunferencia.adaptadores.red.ObservadorServidor;
import juanesteban.circunferencia.adaptadores.red.util.RedUtil;
...
/**
 * Ventana principal de la interfaz gráfica del Servidor (Entrypoint GUI). Responsabilidad única:
 * renderizar la interfaz visual, capturar acciones del usuario e interactuar exclusivamente con
 * los puertos de entrada y observadores de eventos.
 */
public class ServidorFrame extends JFrame implements ObservadorServidor {
  ...
  public ServidorFrame(final GestionarServidorInputPort gestionarServidorPort) {
    super("Servidor UDP - Cálculo de Circunferencia (Arquitectura Hexagonal / SOLID)");
    this.gestionarServidorPort =
  ...
  private JPanel crearPanelNorte() {
    ...
    final JLabel lblTitulo = new JLabel("Servidor de Cálculo de Circunferencia (Protocolo UDP)");
  ...
  private JPanel crearPanelCentral() {
    ...
    final JLabel lblLogsTitulo =
        new JLabel("Registro de Eventos y Conexiones (fecha-hora: IP:Puerto [datos recibidos/enviados]):");
  ...
  public void onEvento(final EventoServidor evento) {
    ...
    if ("EVENTO".equalsIgnoreCase(evento.getCategoria())) {
      linea =
          String.format("%s: %s (%s)%n", timestamp, evento.getEndpoint(), evento.getDescripcion());
    } else {
      linea =
          String.format(
              "%s: [SERVIDOR] [%s] %s%n",
