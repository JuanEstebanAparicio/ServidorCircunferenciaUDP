...
/** Encapsula la administración y las operaciones sobre el DatagramSocket del sistema operativo. */
public class CanalUdp {
  ...
  /**
   * Envía una respuesta usando el socket propiedad del canal.
   *
   * <p>El socket se toma prestado y no debe cerrarse en este método, porque su ciclo de vida
   * abarca todas las operaciones realizadas entre {@link #abrir(int)} y {@link #cerrar()}.
   */
  ...
  public void enviar(final UdpResponse respuesta) throws IOException {
    ...
    final DatagramPacket paquete =
        new DatagramPacket(buffer, buffer.length, direccion, respuesta.getPuertoDestino());
    socketAbierto.send(paquete);
  }

  /**
   * Recibe un paquete usando el socket propiedad del canal.
   *
   * <p>El socket se toma prestado; {@link #cerrar()} conserva la responsabilidad exclusiva de
   * liberar el recurso y de interrumpir una recepción bloqueada.
   */
  @SuppressWarnings("resource")
