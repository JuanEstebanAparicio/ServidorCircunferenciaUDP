...
/**
 * Servicio de aplicación para procesar las peticiones del protocolo UDP de la circunferencia.
 * Responsabilidad única: decodificar y orquestar el procesamiento de los comandos de la red,
 * coordinar con el dominio y despachar respuestas a través de los puertos de salida.
 */
public final class ProcesarPeticionUdpService implements ProcesarPeticionUdpInputPort {
  ...
  public ProcesarPeticionUdpService(
    ...
    this.puertoSalidaRed =
        Objects.requireNonNull(puertoSalidaRed, "El puerto de salida es obligatorio.");
    this.puertoNotificacion =
        Objects.requireNonNull(puertoNotificacion, "El puerto de notificación es obligatorio.");
    this.peticionMapper =
  ...
  public void procesar(final ProcesarPeticionUdpCommand comando) {
    ...
    if (comandoTexto.equalsIgnoreCase("CONECTAR")) {
      puertoSalidaRed.enviarRespuesta(
          RespuestaCliente.conectado(
              destinatario, "Servidor UDP listo para recibir cálculos de circunferencia"));
      notificarEvento(
    ...
    notificarEvento(
        destinatario.endpoint(), "datos recibidos --> Comando desconocido: [" + comandoTexto + "]");
  }
  ...
  private void procesarCalculo(final Destinatario destinatario, final String comandoTexto) {
    ...
    if (partes.length != 2) {
      puertoSalidaRed.enviarRespuesta(
          RespuestaCliente.error(destinatario, "Formato inválido. Se esperaba CALCULAR;radio"));
      notificarEvento(
          destinatario.endpoint(), "datos recibidos --> Formato incorrecto: " + comandoTexto);
      return;
    }

    try {
      final double valorRadio = Double.parseDouble(partes[1].trim().replace(',', '.'));

      final Calculo calculo = calculoMapper.toDomain(valorRadio);
      ...
      final String logInfo =
          String.format(
              Locale.US,
              "datos recibidos (Radio: %.2f) --> datos enviados (Longitud: %s)",
              calculo.getRadio().valor(),
              resultado.getLongitudFormateada());

      notificarEvento(destinatario.endpoint(), logInfo);

    } catch (final NumberFormatException excepcion) {
      puertoSalidaRed.enviarRespuesta(
          RespuestaCliente.error(destinatario, "El parámetro del radio debe ser numérico."));
      notificarEvento(
          destinatario.endpoint(),
